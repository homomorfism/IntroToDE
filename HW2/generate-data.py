...
OUT = Path("data"); (OUT / "events").mkdir(parents=True, exist_ok=True)
...
with open(OUT / "users.jsonl", "w") as f:
    for uid in range(1, NUM_USERS + 1):
        rec = {
            "id": uid,
            "signup_date": (START - timedelta(days=random.randint(1, 400))).date().isoformat(),
...
with open(OUT / "items.jsonl", "w") as f:
    for iid in range(1, NUM_ITEMS + 1):
        rec = {
            ...
            "tags": random.sample(["sale","new","clearance","gift","popular"], k=random.randint(1,3))
        }
        f.write(json.dumps(rec) + "\n")
...
def event_row(i):
    ...
    rec = {
        ...
        "context": {
            "country": random.choices(countries, weights=[30,10,25,8,10,7,5,5])[0],  # skewed
            "device": random.choice(devices),
            "locale": random.choice(["en_US","en_GB","de_DE","th_TH","vi_VN","fr_FR","pt_BR","hi_IN"]),
            "session_id": fake.uuid4()
        },
        "props": {
            "price": round(random.uniform(3, 300), 2) if et == "purchase" else None,
            "payment_method": random.choice(["card","wallet","cod"]) if et == "purchase" else None,
